...
📦 Módulo: Producto
* Listar todos: GET /api/producto
* Buscar por nombre o ID: GET /api/producto/buscar?termino={nombre_o_id}
* Obtener por ID: GET /api/producto/{id}
* Crear nuevo: POST /api/producto
* Actualizar (Flexible): PUT /api/producto/{id} - (Nota: Podés enviar solo los campos que quieras modificar. El sistema recalcula precios automáticamente si envías costo o margen).
* Eliminar: DELETE /api/producto/{id}
